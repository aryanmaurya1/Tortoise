...
func modifiedRow(n float32, row []float32) []float32 {
	// modifies randomly choosen row, using seed n.
	arr := make([]float32, len(row))
	for i := 0; i < len(row); i++ {
		arr[i] = row[i] * 0.5 * n
	}
	return arr
...
func fillCentroids(data *[][]float32, centroids *[][]float32) {
...
func (m *KMeansModel) Init(name string, nCentroids int) {
	...
	m.Centroids = make([][]float32, nCentroids)
	m.Clusters = make([][]Point, nCentroids)
	fillCentroids(&m.Data.Rows, &m.Centroids)
}

func (m *KMeansModel) updateCentroids() {
	var newCentroid = make([][]float32, len(m.Centroids), len(m.Centroids))
	for i := 0; i < len(newCentroid); i++ {
		newCentroid[i] = make([]float32, len(m.Centroids[0]), len(m.Centroids[0]))
		//for j := 0; j < len(m.Clusters); j++ {
		for k := 0; k < len(m.Clusters[i]); k++ {
			rowIndex := (m.Clusters[i][k]).rowIndex
			newCentroid[i] = RowiseSum(newCentroid[i], m.Data.Rows[rowIndex])
		}
		//}
		for k := 0; k < len(newCentroid[i]); k++ {
			newCentroid[i][k] /= float32(len(m.Clusters[i]))
		}
	}
...
func (m *KMeansModel) singlePass() {
	for i := 0; i < len(m.Data.Rows); i++ {
		var minDist float32 = 1000000000.0 // Pseudo +INF
